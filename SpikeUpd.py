...
def spike_graph_plot_descriptive(
    ...
    # update position for the plot
    if runs_count:
        ax.text(70, 420, f"{total_score} ({balls_faced} balls)",
                fontsize=11, ha='center', fontweight='bold')
        
    if show_fours_sixes:
        ax.text(160, 420, f" - 4s: {total_4s} | 6s: {total_6s}",
                fontsize=11, ha='center', color='darkgreen')
    ...
    if show_bowler:
        if bowler_name is None:
            bowler_name = 'All Bowlers'
        if bowler_name:
            ax.text(265,420, f" - vs {bowler_name}",
                    fontsize=11, ha='center', color='blue', fontweight='bold')

    if show_title:
        ax.text(180,385,title_text, fontsize=12, fontweight='bold',  ha='center',
                     fontfamily='DejaVu Sans')

    if show_control:
        ax.text(180, 405, f"Control: {control_pct}%",
                fontsize=12, ha='center', color='purple', fontweight='bold')

    if show_prod_shot:
        ax.text(180, 440, f"Productive Shot: {most_prod_shot_text}",
                fontsize=11, ha='center', color='navy',fontweight='bold')
    ...
    if show_legend:
        ...
        ax.legend(
            handles=legend_elements,
            loc='center',
            bbox_to_anchor=(180, 460),  # (x, y) in data coordinates
            bbox_transform=ax.transData,
            ncol=4,
            frameon=False,
            fontsize=8
