...
# transparent wagon zone plot
def wagon_zone_plot_transparent(
    df, player_name=None, inns=None, mat_num=None, bowler_name=None, team_bat=None, 
    team_bowl=None, run_values=None, competition=None, transparent=True, 
    date_from=None, date_to=None,
    show_title=True, show_summary=True,show_fours_sixes=True, show_control=True, 
    show_prod_shot=True,runs_count=True, show_bowler=True
):
...
def wagon_zone_plot_descriptive(
    ...
    #  update the positions of the summary texts
    if runs_count:
        ax.text(80, 410, f"{total_score} ({balls_faced} balls)",
                fontsize=11, ha='center', fontweight='bold')

    if show_fours_sixes:
        ax.text(170, 410, f" - 4s: {int(player_data['isFour'].sum())} | 6s: {int(player_data['isSix'].sum())}",
                fontsize=11, ha='center', color='darkgreen')

    if show_title:
        ax.text(180, 365, title_text, fontsize=12, ha='center', 
            fontweight='bold', fontfamily='DejaVu Sans')
    
    if show_control:
        ax.text(180, 390, f"Control: {control_pct}%", fontsize=12, ha='center',
                color='purple', fontweight='bold')

    if show_prod_shot:
        ax.text(180, 430, f"Productive Shot: {most_prod_shot_text}", fontsize=11,
                ha='center', color='navy', fontweight='bold')

    if show_bowler:
        if bowler_name is None:
            bowler_name = 'All Bowlers'
        ax.text(265, 410, f" - vs {bowler_name}", fontsize=11, ha='center',
                color='blue', fontweight='bold')
