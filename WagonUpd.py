...
def wagon_zone_plot_descriptive(
    ...
    for i in range(8):
        ...
        label_x = center_x + 100 * np.cos(mid_angle)
        label_y = center_y + 100 * np.sin(mid_angle)
        ax.text(label_x, label_y, f"{quadrant_totals[i]} runs", fontsize=12.5,
                color='black' if i in top_quadrants else '#000',
                ha='center', va='center',fontweight = 'bold')
    ...
    #  update the positions of the summary texts
    if runs_count:
        ax.text(60, 475, f"{total_score} ({balls_faced} balls)",
                fontsize=11, ha='center', fontweight='bold')

    if show_fours_sixes:
        ax.text(170, 475, f" - 4s: {int(player_data['isFour'].sum())} | 6s: {int(player_data['isSix'].sum())}",
                fontsize=11, ha='center', color='darkgreen')
    ...
    if show_bowler:
        if bowler_name is None:
            bowler_name = 'All Bowlers'
        ax.text(285, 475, f" - vs {bowler_name}", fontsize=11, ha='center',
                color='blue', fontweight='bold')
...
def wagon_zone_plot_descriptive_transparent(
    df, player_name=None, inns=None, mat_num=None, bowler_name=None, team_bat=None, 
    team_bowl=None, run_values=None, competition=None, transparent=True, 
    date_from=None, date_to=None,
    show_title=True, show_summary=True,show_fours_sixes=True, show_control=True, 
